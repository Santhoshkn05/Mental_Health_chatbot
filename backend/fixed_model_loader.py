...
# -*- coding: utf-8 -*-
"""
FIXED MODEL LOADER - CLEAN VERSION (No Context Handling)
Restored to working state before context handling was added
Updated with DialoGPT Fixes & Qwen2.5-0.5B Refinement/Fallback Integration
"""
...
MODEL_PATHS = {
    ...
    "phi3-mini": os.path.join(MODELS_DIR, "phi3-mini"),
    "qwen": os.path.join(MODELS_DIR, "Qwen2.5-0.5B-Instruct"), # Added Qwen Path
}
...
class FixedMentalHealthBot:
    def __init__(self, model_name="esconv-dialo-final"):
        ...
        self._load_qwen() # Automatically load Qwen for refinement
        # self._load_phi3_mini()  # Temporarily disabled due to download issues
    ...
    def _get_device(self):
        ...
        if hasattr(torch.version, 'hip') and torch.version.hip:
            ...
            return torch.device("cuda")  # ROCm still uses "cuda" device
    ...
    def _load_intent_model(self):
        try:
            ...
            print("[OK] Intent model loaded successfully")

            test_output = self.predict_intent("hello")
            print(f"[TEST] Test prediction: {test_output}")

        except Exception as e:
            print(f"[ERROR] Intent model failed: {e}")
            self.model_status["intent"] = False

    def _load_emotion_model(self):
        try:
            print("\n[LOAD] Loading Emotion Model...")
    ...
    def _load_risk_model(self):
        try:
            ...
            print(f"[ERROR] Risk model failed: {e}")
        ...
        for model_name in response_models:
            ...
            try:
                ...
                if not model_path or not os.path.exists(model_path):
                    print(f"[SKIP] Skipping {model_name}: path not found")
                    continue
                ...
                try:
                    ...
                    print(f"[OK] GPT2 tokenizer loaded successfully")
                    ...
                    print(f"[ERROR] All tokenizer attempts failed: {tokenizer_error}")
                    self.resp_tokenizer = None
                    continue
    ...
    def _load_phi3_mini(self):
        print("[LOAD] Loading Phi-3 Mini model...")

        try:
            phi3_path = MODEL_PATHS.get("phi3-mini")
            is_cuda = self.device.type == "cuda"

            # ===============================
            # 1. LOAD FROM LOCAL IF EXISTS
            # ===============================
            if phi3_path and os.path.exists(phi3_path):
                print("[LOAD] Loading Phi-3 from LOCAL path")

                self.phi3_tokenizer = AutoTokenizer.from_pretrained(phi3_path)
            ...
            # ===============================
            # 2. ELSE LOAD FROM HUGGINGFACE
            # ===============================
            else:
                print("[LOAD] Loading Phi-3 from HuggingFace")

                # Try different Phi-3 model variants - prioritize smaller models for CPU
                phi3_models = [
                ...
                loaded = False
                for model_name in phi3_models:
                    if loaded:
                        break
                        
                    print(f"[LOAD] Trying model: {model_name}")
                    try:
                        # First try without trust_remote_code
                        try:
                            self.phi3_tokenizer = AutoTokenizer.from_pretrained(model_name)
                            
                            # Use different loading strategies for CPU vs GPU
                            if is_cuda:
                                self.phi3_model = AutoModelForCausalLM.from_pretrained(
                                    model_name,
                            ...
                            loaded = True
                            print(f"[OK] Loaded {model_name} without trust_remote_code")
                        except Exception as no_trust_error:
                            print(f"[WARN] Standard loading failed for {model_name}: {no_trust_error}")
                            
                            # Try with trust_remote_code and memory optimization
                            try:
                                ...
                                loaded = True
                                print(f"[OK] Loaded {model_name} with trust_remote_code")
                            except Exception as trust_error:
                                print(f"[ERROR] Trust loading failed for {model_name}: {trust_error}")
                                continue
                        
                    except Exception as model_error:
                        print(f"[ERROR] Model {model_name} failed: {model_error}")
                        continue
                
                if not loaded:
                    raise Exception("All Phi-3 model variants failed to load")

                # Only move manually if CPU
                if not is_cuda and hasattr(self.phi3_model, 'to'):
                    self.phi3_model = self.phi3_model.to(self.device)

            # ===============================
            # 3. FIX TOKENIZER
            # ===============================
            if self.phi3_tokenizer and hasattr(self.phi3_tokenizer, 'pad_token'):
                if self.phi3_tokenizer.pad_token is None:
                    self.phi3_tokenizer.pad_token = self.phi3_tokenizer.eos_token

            self.model_status["phi3_mini"] = True
            print("[OK] Phi-3 Mini loaded successfully")

        except Exception as e:
            print(f"[ERROR] Phi-3 Mini load failed: {e}")
            self.phi3_model = None
            self.phi3_tokenizer = None
            self.model_status["phi3_mini"] = False
            print("[SKIP] Continuing without Phi-3 Mini")
    ...
    def predict_risk(self, text):
        ...
        # EXPANDED high-risk keywords (comprehensive crisis detection)
        high_risk_keywords = [
            ...
            # Self-harm (CRITICAL: Added "thinking about self harm")
            "harm myself", "hurt myself", "self harm", "self-harm",
            "cut myself", "mutilate", "wound myself", "injure myself",
            "thinking about self harm", "thinking of self harm", "considering self harm",
            
            # Hopelessness/despair (CRITICAL: Added "lost all hope")
            "no reason to live", "no point living", "pointless",
            "hopeless", "hopelessness", "can't go on",
            "cant go on", "give up", "lost all hope", "lost hope",
            
            # Burden statements (CRITICAL: Added "burden to everyone")
            "burden", "burden to others", "burden to everyone", "better off dead",
            "world better without me", "nobody needs me", "everyone better off",
            
            # Goodbyes/final statements
            "goodbye forever", "final goodbye", "goodbye world",
            "farewell", "see you never", "last message",
            
            # Escape/disappearance
            "want to vanish", "disappear", "go away forever",
            "run away", "escape this life",
            
            # Pain/suffering extremes
            "pain too much", "suffering too much",
            "can't take it anymore", "unbearable pain",
            "extreme physical pain", "unbearable mental pain",
            
            # Detailed planning (very high risk)
            "planning to", "going to", "going to kill",
            "method", "ways to", "how to",
            "already decided", "made my decision",
            
            # Recent loss/crisis
            "just lost", "just found out", "can't handle",
            "too much", "breaking point",
        ]

        # Count and score high-risk keywords
        keyword_hits = sum(1 for keyword in high_risk_keywords if keyword in text_lower)
        ...
        # Combine scores (take maximum - more cautious approach)
        final_score = max(ml_score, rule_score)
        label = "high_risk" if final_score > 0.6 else "low_risk"
    ...
    def is_relevant_input(self, text):
        ...
        crisis_keywords = ["suicide", "kill myself", "kill", "die", "death", "end", "harm", "crisis", "emergency"]
        ...
        if has_crisis_keywords:
            print(f"[CRISIS] Crisis keywords detected - bypassing filters")
            return True
        
        # REJECT very short inputs (1-2 words) for non-crisis content
        if word_count <= 2:
            print(f"[FILTER] Input too short ({word_count} words) - rejected")
            return False
    ...
    def refine_response(self, user_input, response, emotion):
        ...
        # 2. Try Phi-3 Mini refinement if Qwen failed/unavailable
        elif self.model_status.get("phi3_mini") and self.phi3_model is not None:
            try:
                return self._refine_with_phi3(user_input, response, emotion)
            except Exception as e:
                print(f"[PHI3] Refinement failed: {e}, falling back to basic refinement")
        
        # 3. Enhanced refinement with sophisticated empathy
        empathy_responses = {
            'sadness': [
    ...
    def _refine_with_phi3(self, user_input, response, emotion):
        """
        Use Phi-3 Mini to refine and validate the response.
        """
        refinement_prompt = f"""You are a supportive mental health assistant. 
...
2. More natural and conversational
3. Better tailored to the user's emotional state
4. Include a gentle follow-up question if appropriate
        ...
        try:
            ...
            with torch.no_grad():
                outputs = self.phi3_model.generate(
                    **inputs,
                    max_new_tokens=80,
                    do_sample=True,           # 🔥 FIXED: Requires True to use temperature
                    temperature=0.2,          
            ...
            refined_response = self.phi3_tokenizer.decode(
                outputs[0], 
                skip_special_tokens=True
            ).strip()
            
            # Extract only the refined response part
            if "Refined response:" in refined_response:
                refined_response = refined_response.split("Refined response:")[-1].strip()
            
            print(f"[PHI3] Refined: {refined_response}")
            return refined_response
            
        except Exception as e:
            print(f"[PHI3] Error during refinement: {e}")
            raise e
...
Score:"""
    ...
    def generate_simple_response(self, text, intent, emotion, history=None):
        """
        ENHANCED response generation v4.0 without context handling.
        
        Pipeline:
        1. Domain filter: Reject irrelevant/very short input
        2. Crisis check: Detect high-risk situations  
        3. Generate response with improved settings
        4. Clean response -> clean_response()
        5. Check quality -> is_meaningful()
        6. Refine response -> refine_response()
        7. Return final response
        
        Returns: str (validated and refined response text)
        """
        try:
            # STEP 1: Crisis detection using ML classifier (CHECK FIRST)
            ...
            if risk_label == "high_risk":
                ...
                return self.generate_crisis_response(text, user_context)
            
            # STEP 2: Domain filtering (only for non-crisis content)
            if not self.is_relevant_input(text):
                print("[BLOCK] Input filtered - not relevant to mental health domain")
                return self.irrelevant_topic_response()
            
            # STEP 3: Use Qwen as primary response generator for better quality
            # Extract current user input from context
            if isinstance(text, str) and "\n" in text:
                ...
                for line in reversed(lines):
                    if line.startswith("User:"):
                        ...
                        break
                    elif not line.startswith("Assistant:"):
                        current_input = line.strip()
                        break
                else:
                    current_input = text.split('\n')[-1].strip()
            else:
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    # Allow multiple questions - remove fragmentation check
                    ...
                    # Additional validation with Phi-3 Mini if available
                    phi3_valid = True
                    if self.model_status.get("phi3_mini") and self.phi3_model is not None:
                        try:
                            phi3_valid, phi3_score, phi3_feedback = self.validate_with_phi3(text, response_text, emotion)
                            if phi3_score < 0.6:  # Low quality threshold
                                is_meaningful = False
                                print(f"[PHI3] Quality too low: {phi3_score:.2f} - {phi3_feedback}")
                        except Exception as e:
                            print(f"[PHI3] Validation error: {e}")
                    
                    if is_meaningful and phi3_valid and not is_hallucinated:
                        print("[SUCCESS] Response is meaningful - proceeding to refinement")
                        
                        # STEP 7: Refine response with Qwen (or Phi-3)
